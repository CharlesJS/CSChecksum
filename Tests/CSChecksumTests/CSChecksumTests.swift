...
@Suite
struct ExcessivelyLongInputData {
    static let excessivelyLongData: Data = {
    ...
    init() {
        _ = Self.excessivelyLongData
    }

    @Test
    func testCRC32OnVeryLongData() throws {
        let expected = Data([0x56, 0x28, 0xe3, 0xe5])
        #expect(Data(CSChecksum.checksum(for: Self.excessivelyLongData, algorithm: .crc32)) == expected)
    }

    @Test
    func testCRC32COnVeryLongData() throws {
        let expected = Data([0x3a, 0x4a, 0x25, 0x33])
        #expect(Data(CSChecksum.checksum(for: Self.excessivelyLongData, algorithm: .crc32c)) == expected)
    }

#if Crypto
#if canImport(CommonCrypto)
    @Test
    func testSHA224OnVeryLongData() {
        let expected = Data([
        ...
        #expect(Data(CSChecksum.checksum(for: Self.excessivelyLongData, algorithm: .sha224)) == expected)
    }
#endif

    @Test
    func testSHA512OnVeryLongData() {
        let expected = Data([
        ...
        #expect(Data(CSChecksum.checksum(for: Self.excessivelyLongData, algorithm: .sha512)) == expected)
    }
#endif
}
