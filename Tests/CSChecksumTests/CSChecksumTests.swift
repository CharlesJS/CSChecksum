...
final class CSChecksumTests: XCTestCase {
    ...
    func testExcessivelyLongInputData() {
        ...
        let expectedChecksums: [CSChecksum.Algorithm : Data] = [
        ...
        for (algorithm, expected) in expectedChecksums {
            NSLog("getting for \(algorithm)")
            XCTAssertEqual(Data(CSChecksum.checksum(for: data, algorithm: algorithm)), expected)
            NSLog("got \((Data(CSChecksum.checksum(for: data, algorithm: algorithm)) as NSData).debugDescription)) for \(algorithm)")
        }
    }
